...
const AttachedFiles: React.FC<AttachedFilesProps> = ({
  ...
  return filesAttached ? (
    <div className={`${formClass}-attached-files-comp`}>
      {typeof filesAttached === "string" ? (
        <span>{filesAttached}</span>
      ) : (
        <span>{filesAttached} files attached</span>
      )}
      <img
        className={`${formClass}-attached-files-clearing-cross`}
        src={clearAttachedFiles}
        alt="clear attached files"
        onClick={handleClickClearingAttachedFiles}
      />
    </div>
  ) : null;
};
