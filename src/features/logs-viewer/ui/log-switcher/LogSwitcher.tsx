...
const LogSwitcher: React.FC<LogSwitcherProps> = ({
  firstShownIndex,
  setFirstShownIndex,
}) => {
  ...
  const handleNextClick = () => {
    setFirstShownIndex((prev) =>
      prev + 10 <= logsMatches[fileName].length - 1 ? prev + 10 : prev,
    );
  };
  ...
  return (
    <div className="main__log-switcher">
      <div
        className={`main__log-switcher-prev ${firstShownIndex - 10 < 0 ? " main__log-switcher-prev_disabled" : ""}`}
        onClick={handlePrevClick}
      >
        {"<"}
      </div>
      <div
        className={`main__log-switcher-next ${firstShownIndex + 10 > logsMatches[fileName]?.length - 1 ? " main__log-switcher-next_disabled" : ""}`}
