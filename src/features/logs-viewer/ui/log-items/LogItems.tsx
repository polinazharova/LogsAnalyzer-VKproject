...
const LogItems: React.FC<LogItemsProps> = ({logs, firstShownIndex, setFirstShownIndex}) => {
    const {selectedLogsType} = useContext(SelectedLogsTypeContext);
    const {selectedFile} = useContext(SelectedFileContext);

    const fileName = useMemo(() => {
      return Object.keys(logs).includes(selectedFile)  ? selectedFile : Object.keys(logs)[0];
    }, [selectedFile]);

    const {logsMatches, setLogsMatches} = useContext(LogsMatchesContext);

    const matchesTmp = useMatches({logs, fileName, selectedLogsType, setFirstShownIndex});
    useEffect(() => {
        ...
        <div className="main__log-viewer-items">
            {
              ...
              (logsMatches[fileName].slice(firstShownIndex, firstShownIndex + 10).map((log : string, index : number) => (
                <LogItem key={fileName + index} fileName={fileName} date={log[0]} message={log[1]}/>
              ))) 
                : 
                (logs[fileName].length ? <div key={`${fileName}-wrong`}>Wrong format of logs!</div> : <div className="main__log-viewer-item" key={`${fileName}-empty`}>{fileName} is empty</div>)
            }
        </div>
      )
    }
