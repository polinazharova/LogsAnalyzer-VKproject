...
const SelectedLogsTypeProvider : React.FC<SelectedLogsTypeProviderProps> = ({children}) => {

    const [selectedLogsType, setSelectedLogsType] = useState<string>("Select a logs type");

    return (
        <SelectedLogsTypeContext.Provider value={{selectedLogsType, setSelectedLogsType}}>{children}</SelectedLogsTypeContext.Provider>
    )
}
