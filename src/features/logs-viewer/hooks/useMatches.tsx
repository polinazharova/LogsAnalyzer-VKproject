...
const useMatches = ({logs, fileName, selectedLogsType, setFirstShownIndex} : UseMatchesParams) => {
    useEffect(() => {
      ...
      const matches : any = useMemo(() => {
          ...
          logs[fileName].forEach((logBody : string) => {
            ...
            if (match) {
              if (match[2] !== selectedLogsType && selectedLogsType !== 'ALL' && selectedLogsType !== 'Select a logs type') {
                return;
              }
