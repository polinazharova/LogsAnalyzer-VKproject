...
const useLogsTypeSelectorHandlers = (setIsSelectorItemsHidden : React.Dispatch<React.SetStateAction<boolean>>, setSelectedLogsType: React.Dispatch<React.SetStateAction<string>>) => {
    ...
    const handleSelectorItemClick = (event: Event) : void => {
        ...
        const prevSelectedItem = document.querySelector('.main__logs-type-selector-item_selected');

        if (prevSelectedItem) {
            prevSelectedItem.classList.remove('main__logs-type-selector-item_selected');
        }
    ...
    useEffect(() => {
        ...
        const selectorItemArr = document.querySelectorAll(".main__logs-type-selector-item");
