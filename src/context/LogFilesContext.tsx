...
const LogFilesContext = createContext<LogFilesContextType>({logFiles: null, setLogFiles: () : void => {}});
...
const LogFilesProvider : React.FC<LogFilesProviderProps> = ({children}) => {
    ...
    return (
        <LogFilesContext.Provider value={{logFiles, setLogFiles}}>{children}</LogFilesContext.Provider>
    )
}
